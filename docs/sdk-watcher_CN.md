...
- `watcher.AuthUpdate`描述单条凭据变更，`Action`可能为`add`、`modify`或`delete`，`ID`是凭据标识。对于`add`/`modify`会携带完整的
  `Auth`克隆，`delete`可以省略`Auth`。
...
- `internal/watcher/watcher.go`维护`currentAuths`快照，文件或配置事件触发后会重建快照并与旧快照对比，生成最小化的
  `AuthUpdate`列表。
...
- 背压通过两级缓冲吸收：
    ...
    - 服务端通道的 256 容量加上消费侧的”抽干”逻辑，可平稳处理多个突发批次。
