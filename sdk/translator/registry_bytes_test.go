...
func TestRegistryTranslateStreamReturnsByteChunks(t *testing.T) {
	...
	registry.Register(
		FormatOpenAI, FormatGemini, nil, ResponseTransform{
			Stream: func(
				ctx context.Context,
				model string,
	...
	got := registry.TranslateStream(
		context.Background(), FormatGemini, FormatOpenAI, "model", nil, nil, []byte(`{"chunk":true}`), nil,
	)
	if len(got) != 1 {
...
func TestRegistryTranslateNonStreamReturnsBytes(t *testing.T) {
	...
	got := registry.TranslateNonStream(
		context.Background(), FormatGemini, FormatOpenAI, "model", nil, nil, []byte(`{"done":true}`), nil,
	)
...
func TestRegistryTranslateTokenCountReturnsBytes(t *testing.T) {
	...
	got := registry.TranslateTokenCount(
		context.Background(), FormatGemini, FormatOpenAI, 7, []byte(`{"fallback":true}`),
	)
