...
func (m *configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(
		fields,
		configField{"Request Retry", "request-retry", "int", fmt.Sprintf("%.0f", getFloat(cfg, "request-retry")), nil},
	)
	fields = append(
		fields, configField{
			"Max Retry Interval (s)", "max-retry-interval", "int",
			fmt.Sprintf("%.0f", getFloat(cfg, "max-retry-interval")), nil,
		},
	)
	fields = append(
		fields,
		configField{"Force Model Prefix", "force-model-prefix", "string", getString(cfg, "force-model-prefix"), nil},
	)

	// Logging
	fields = append(
		fields, configField{
			"Logging to File", "logging-to-file", "bool", fmt.Sprintf("%v", getBool(cfg, "logging-to-file")), nil,
		},
	)
	fields = append(
		fields, configField{
			"Logs Max Total Size (MB)", "logs-max-total-size-mb", "int",
			fmt.Sprintf("%.0f", getFloat(cfg, "logs-max-total-size-mb")), nil,
		},
	)
	fields = append(
		fields, configField{
			"Error Logs Max Files", "error-logs-max-files", "int",
			fmt.Sprintf("%.0f", getFloat(cfg, "error-logs-max-files")), nil,
		},
	)
	fields = append(
		fields, configField{
			"Usage Stats Enabled", "usage-statistics-enabled", "bool",
			fmt.Sprintf("%v", getBool(cfg, "usage-statistics-enabled")), nil,
		},
	)
	fields = append(
		fields, configField{"Request Log", "request-log", "bool", fmt.Sprintf("%v", getBool(cfg, "request-log")), nil},
	)

	// Quota exceeded
	fields = append(
		fields, configField{
			"Switch Project on Quota", "quota-exceeded/switch-project", "bool",
			fmt.Sprintf("%v", getBoolNested(cfg, "quota-exceeded", "switch-project")), nil,
		},
	)
	fields = append(
		fields, configField{
			"Switch Preview Model", "quota-exceeded/switch-preview-model", "bool",
			fmt.Sprintf("%v", getBoolNested(cfg, "quota-exceeded", "switch-preview-model")), nil,
		},
	)
	...
	if routing, ok := cfg["routing"].(map[string]any); ok {
		fields = append(
			fields, configField{"Routing Strategy", "routing/strategy", "string", getString(routing, "strategy"), nil},
		)
	} else {
	...
	// WebSocket auth
	fields = append(
		fields, configField{"WebSocket Auth", "ws-auth", "bool", fmt.Sprintf("%v", getBool(cfg, "ws-auth")), nil},
	)
	...
	if amp, ok := cfg["ampcode"].(map[string]any); ok {
		...
		fields = append(
			fields, configField{"AMP Upstream URL", "ampcode/upstream-url", "string", upstreamURL, upstreamURL},
		)
		fields = append(
			fields, configField{
				"AMP Upstream API Key", "ampcode/upstream-api-key", "string", maskIfNotEmpty(upstreamAPIKey),
				upstreamAPIKey,
			},
		)
		fields = append(
			fields, configField{
				"AMP Restrict Mgmt Localhost", "ampcode/restrict-management-to-localhost", "bool",
				fmt.Sprintf("%v", getBool(amp, "restrict-management-to-localhost")), nil,
			},
		)
	}
