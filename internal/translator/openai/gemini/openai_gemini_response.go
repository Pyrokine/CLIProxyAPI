...
func ConvertOpenAIResponseToGemini(_ context.Context, _ string, _, _, rawJSON []byte, param *any) [][]byte {
	if *param == nil {
			...
			func(choiceIndex, choice gjson.Result) bool {
				...
				if reasoning := delta.Get("reasoning_content"); reasoning.Exists() {
					for _, reasoningText := range extractReasoningTexts(reasoning) {
						...
						reasoningTemplate, _ = sjson.SetBytes(
				...
				if content := delta.Get("content"); content.Exists() && content.String() != "" {
					...
					contentTemplate, _ = sjson.SetBytes(
						contentTemplate, "candidates.0.content.parts.0.text", contentText,
					)
					...
					if len((*param).(*ConvertOpenAIResponseToGeminiParams).ToolCallsAccumulator) > 0 {
						...
						for _, accumulator := range (*param).(*ConvertOpenAIResponseToGeminiParams).ToolCallsAccumulator {
							...
							template, _ = sjson.SetRawBytes(
								template, argsPath, []byte(parseArgsToObjectRaw(accumulator.Arguments.String())),
							)
							partIndex++
						}
				...
				if usage := root.Get("usage"); usage.Exists() {
					...
					template, _ = sjson.SetBytes(
						template, "usageMetadata.totalTokenCount", usage.Get("total_tokens").Int(),
					)
					if reasoningTokens := reasoningTokensFromUsage(usage); reasoningTokens > 0 {
						template, _ = sjson.SetBytes(template, "usageMetadata.thoughtsTokenCount", reasoningTokens)
					}
...
func ConvertOpenAIResponseToGeminiNonStream(_ context.Context, _ string, _, _, rawJSON []byte, _ *any) []byte {
	root := gjson.ParseBytes(rawJSON)
				...
				if reasoning := message.Get("reasoning_content"); reasoning.Exists() {
					for _, reasoningText := range extractReasoningTexts(reasoning) {
						...
						out, _ = sjson.SetBytes(
							out, fmt.Sprintf("candidates.0.content.parts.%d.thought", partIndex), true,
						)
						out, _ = sjson.SetBytes(
