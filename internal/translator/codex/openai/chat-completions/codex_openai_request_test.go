...
func TestMultipleToolCalls(t *testing.T) {
	...
	for i, expectedOutput := range expectedOutputs {
		...
		if items[idx].Get("call_id").String() != expectedCallIDs[i] {
			t.Errorf(
				"item %d: expected call_id '%s', got '%s'", idx, expectedCallIDs[i], items[idx].Get("call_id").String(),
			)
		}
...
func TestNoSpuriousEmptyAssistantMessage(t *testing.T) {
	...
	if len(items) != 3 {
		t.Fatalf(
			"expected 3 input items (user + function_call + function_call_output), got %d: %s", len(items),
			gjson.Get(result, "input").Raw,
		)
	}
...
func TestMultiTurnToolCalling(t *testing.T) {
	...
	if items[3].Get("type").String() != "message" || items[3].Get("role").String() != "assistant" {
		t.Errorf(
			"item 3: expected assistant message, got type=%s role=%s", items[3].Get("type").String(),
			items[3].Get("role").String(),
		)
	}
