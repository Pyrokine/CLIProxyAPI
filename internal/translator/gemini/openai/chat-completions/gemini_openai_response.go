...
func convertGeminiResponseToOpenAI(
	...
	baseTemplate = common.SetTimestamp(rawJSON, &p.UnixTimestamp, baseTemplate, "createTime")
	...
	if usageResult := gjson.GetBytes(rawJSON, "usageMetadata"); usageResult.Exists() {
		baseTemplate = common.SetUsageMetadata(baseTemplate, usageResult)
	}
			...
			func(_, candidate gjson.Result) bool {
				...
				if partsResult.IsArray() {
					...
					for i := range partResults {
						skip, isFunctionCall, updated := common.ProcessContentPart(
							template, partResults[i], prefix, &functionCallIndex,
						)
						if skip {
							continue
						}
...
func ConvertGeminiResponseToOpenAINonStream(
	...
	template = common.SetTimestamp(rawJSON, &unixTimestamp, template, "createTime")
	...
	if usageResult := gjson.GetBytes(rawJSON, "usageMetadata"); usageResult.Exists() {
		template = common.SetUsageMetadata(template, usageResult)
	}
					...
					for i := range partsResults {
						...
						if partTextResult.Exists() {
							...
							if partResult.Get("thought").Bool() {
								oldVal := gjson.Get(choiceTemplate, prefix+".reasoning_content").String()
								choiceTemplate, _ = sjson.Set(
									choiceTemplate, prefix+".reasoning_content", oldVal+partTextResult.String(),
								)
							} else {
								oldVal := gjson.Get(choiceTemplate, prefix+".content").String()
								choiceTemplate, _ = sjson.Set(
									choiceTemplate, prefix+".content", oldVal+partTextResult.String(),
								)
							}
							choiceTemplate, _ = sjson.Set(choiceTemplate, prefix+".role", "assistant")
						} else if functionCallResult.Exists() {
							hasFunctionCall = true
							choiceTemplate = common.AppendFunctionCall(
								choiceTemplate, functionCallResult, prefix, nil,
							)
						} else if inlineDataResult.Exists() {
							if result, ok := common.AppendInlineData(
								choiceTemplate, inlineDataResult, prefix,
							); ok {
								choiceTemplate = result
							}
						}
					}
				}
