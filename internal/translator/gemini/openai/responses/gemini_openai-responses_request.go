			...
			case "message":
				if strings.EqualFold(itemRole, "system") {
					if contentArray := item.Get("content"); contentArray.Exists() {
						...
						if systemInstructionResult := gjson.GetBytes(
			...
			case "function_call_output":
				...
				if outputRaw != "" && outputRaw != "null" {
					...
					if output.Type == gjson.JSON && json.Valid([]byte(output.Raw)) {
						functionResponse, _ = sjson.SetRawBytes(
							functionResponse, "functionResponse.response.result", []byte(output.Raw),
						)
					} else {
						functionResponse, _ = sjson.SetBytes(
							functionResponse, "functionResponse.response.result", outputRaw,
						)
					}
				}
