...
func ConvertGeminiRequestToGemini(_ string, inputRawJSON []byte, _ bool) []byte {
	...
	gjson.GetBytes(out, "contents").ForEach(
					...
					func(partKey, part gjson.Result) bool {
						if part.Get("functionCall").Exists() {
							out, _ = sjson.SetBytes(
								out, fmt.Sprintf("contents.%d.parts.%d.thoughtSignature", key.Int(), partKey.Int()),
							...
							if strings.TrimSpace(name) == "" {
								if ri < len(pendingCallNames) {
									out, _ = sjson.SetBytes(
										out,
										fmt.Sprintf(
											"contents.%d.parts.%d.functionResponse.name", contentIdx.Int(),
											partIdx.Int(),
										),
										pendingCallNames[ri],
									)
									...
									log.Debugf(
										"more function responses than calls at contents[%d], skipping name backfill",
										contentIdx.Int(),
									)
								}
							}
