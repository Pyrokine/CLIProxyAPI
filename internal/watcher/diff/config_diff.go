...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if strings.TrimSpace(oldCfg.Pprof.Addr) != strings.TrimSpace(newCfg.Pprof.Addr) {
		changes = append(
			changes, fmt.Sprintf(
				"pprof.addr: %s -> %s", strings.TrimSpace(oldCfg.Pprof.Addr), strings.TrimSpace(newCfg.Pprof.Addr),
			),
		)
	}
	...
	if oldCfg.UsageStatisticsEnabled != newCfg.UsageStatisticsEnabled {
		changes = append(
			changes, fmt.Sprintf(
				"usage-statistics-enabled: %t -> %t", oldCfg.UsageStatisticsEnabled, newCfg.UsageStatisticsEnabled,
			),
		)
	}
	if oldCfg.DisableCooling != newCfg.DisableCooling {
		changes = append(
			changes, fmt.Sprintf("disable-cooling: %t -> %t", oldCfg.DisableCooling, newCfg.DisableCooling),
		)
	}
	...
	if oldCfg.LogsMaxTotalSizeMB != newCfg.LogsMaxTotalSizeMB {
		changes = append(
			changes,
			fmt.Sprintf("logs-max-total-size-mb: %d -> %d", oldCfg.LogsMaxTotalSizeMB, newCfg.LogsMaxTotalSizeMB),
		)
	}
	if oldCfg.ErrorLogsMaxFiles != newCfg.ErrorLogsMaxFiles {
		changes = append(
			changes, fmt.Sprintf("error-logs-max-files: %d -> %d", oldCfg.ErrorLogsMaxFiles, newCfg.ErrorLogsMaxFiles),
		)
	}
	...
	if oldCfg.MaxRetryCredentials != newCfg.MaxRetryCredentials {
		changes = append(
			changes,
			fmt.Sprintf("max-retry-credentials: %d -> %d", oldCfg.MaxRetryCredentials, newCfg.MaxRetryCredentials),
		)
	}
	if oldCfg.MaxRetryInterval != newCfg.MaxRetryInterval {
		changes = append(
			changes, fmt.Sprintf("max-retry-interval: %d -> %d", oldCfg.MaxRetryInterval, newCfg.MaxRetryInterval),
		)
	}
	if oldCfg.ProxyURL != newCfg.ProxyURL {
		changes = append(
			changes,
			fmt.Sprintf("proxy-url: %s -> %s", formatProxyURL(oldCfg.ProxyURL), formatProxyURL(newCfg.ProxyURL)),
		)
	}
	...
	if oldCfg.ForceModelPrefix != newCfg.ForceModelPrefix {
		changes = append(
			changes, fmt.Sprintf("force-model-prefix: %t -> %t", oldCfg.ForceModelPrefix, newCfg.ForceModelPrefix),
		)
	}
	if oldCfg.NonStreamKeepAliveInterval != newCfg.NonStreamKeepAliveInterval {
		changes = append(
			changes, fmt.Sprintf(
				"nonstream-keepalive-interval: %d -> %d", oldCfg.NonStreamKeepAliveInterval,
				newCfg.NonStreamKeepAliveInterval,
			),
		)
	}
	...
	if oldCfg.QuotaExceeded.SwitchProject != newCfg.QuotaExceeded.SwitchProject {
		changes = append(
			changes, fmt.Sprintf(
				"quota-exceeded.switch-project: %t -> %t", oldCfg.QuotaExceeded.SwitchProject,
				newCfg.QuotaExceeded.SwitchProject,
			),
		)
	}
	if oldCfg.QuotaExceeded.SwitchPreviewModel != newCfg.QuotaExceeded.SwitchPreviewModel {
		changes = append(
			changes, fmt.Sprintf(
				"quota-exceeded.switch-preview-model: %t -> %t", oldCfg.QuotaExceeded.SwitchPreviewModel,
				newCfg.QuotaExceeded.SwitchPreviewModel,
			),
		)
	}
	...
	if oldCfg.Routing.Strategy != newCfg.Routing.Strategy {
		changes = append(
			changes, fmt.Sprintf("routing.strategy: %s -> %s", oldCfg.Routing.Strategy, newCfg.Routing.Strategy),
		)
	}
	...
	if len(oldCfg.GeminiKey) != len(newCfg.GeminiKey) {
		changes = append(
			changes, fmt.Sprintf("gemini-api-key count: %d -> %d", len(oldCfg.GeminiKey), len(newCfg.GeminiKey)),
		)
	} else {
		for i := range oldCfg.GeminiKey {
			...
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(
					changes, fmt.Sprintf(
						"gemini[%d].base-url: %s -> %s", i, strings.TrimSpace(o.BaseURL), strings.TrimSpace(n.BaseURL),
					),
				)
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(
					changes, fmt.Sprintf(
						"gemini[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL),
					),
				)
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(
					changes, fmt.Sprintf(
						"gemini[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix),
					),
				)
			}
			...
			if oldModels.hash != newModels.hash {
				changes = append(
					changes,
					fmt.Sprintf("gemini[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count),
				)
			}
			...
			if oldExcluded.hash != newExcluded.hash {
				changes = append(
					changes, fmt.Sprintf(
						"gemini[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count,
						newExcluded.count,
					),
				)
			}
	...
	if len(oldCfg.ClaudeKey) != len(newCfg.ClaudeKey) {
		changes = append(
			changes, fmt.Sprintf("claude-api-key count: %d -> %d", len(oldCfg.ClaudeKey), len(newCfg.ClaudeKey)),
		)
	} else {
		for i := range oldCfg.ClaudeKey {
			...
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(
					changes, fmt.Sprintf(
						"claude[%d].base-url: %s -> %s", i, strings.TrimSpace(o.BaseURL), strings.TrimSpace(n.BaseURL),
					),
				)
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(
					changes, fmt.Sprintf(
						"claude[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL),
					),
				)
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(
					changes, fmt.Sprintf(
						"claude[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix),
					),
				)
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
			...
			if oldModels.hash != newModels.hash {
				changes = append(
					changes,
					fmt.Sprintf("claude[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count),
				)
			}
			oldExcluded := summarizeExcludedModels(o.ExcludedModels)
			newExcluded := summarizeExcludedModels(n.ExcludedModels)
			if oldExcluded.hash != newExcluded.hash {
				changes = append(
					changes, fmt.Sprintf(
						"claude[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count,
						newExcluded.count,
					),
				)
			}
			if o.Cloak != nil && n.Cloak != nil {
				if strings.TrimSpace(o.Cloak.Mode) != strings.TrimSpace(n.Cloak.Mode) {
					changes = append(
						changes, fmt.Sprintf("claude[%d].cloak.mode: %s -> %s", i, o.Cloak.Mode, n.Cloak.Mode),
					)
				}
				if o.Cloak.StrictMode != n.Cloak.StrictMode {
					changes = append(
						changes,
						fmt.Sprintf(
							"claude[%d].cloak.strict-mode: %t -> %t", i, o.Cloak.StrictMode, n.Cloak.StrictMode,
						),
					)
				}
				if len(o.Cloak.SensitiveWords) != len(n.Cloak.SensitiveWords) {
					changes = append(
						changes, fmt.Sprintf(
							"claude[%d].cloak.sensitive-words: %d -> %d", i, len(o.Cloak.SensitiveWords),
	...
	if len(oldCfg.CodexKey) != len(newCfg.CodexKey) {
		changes = append(
			changes, fmt.Sprintf("codex-api-key count: %d -> %d", len(oldCfg.CodexKey), len(newCfg.CodexKey)),
		)
	} else {
		for i := range oldCfg.CodexKey {
			...
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(
					changes, fmt.Sprintf(
						"codex[%d].base-url: %s -> %s", i, strings.TrimSpace(o.BaseURL), strings.TrimSpace(n.BaseURL),
					),
				)
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(
					changes, fmt.Sprintf(
						"codex[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL),
					),
				)
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(
					changes, fmt.Sprintf(
						"codex[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix),
					),
				)
			}
			...
			if oldModels.hash != newModels.hash {
				changes = append(
					changes,
					fmt.Sprintf("codex[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count),
				)
			}
			oldExcluded := summarizeExcludedModels(o.ExcludedModels)
			newExcluded := summarizeExcludedModels(n.ExcludedModels)
			if oldExcluded.hash != newExcluded.hash {
				changes = append(
					changes, fmt.Sprintf(
						"codex[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count,
						newExcluded.count,
					),
				)
			}
	...
	if oldCfg.AmpCode.RestrictManagementToLocalhost != newCfg.AmpCode.RestrictManagementToLocalhost {
		changes = append(
			changes, fmt.Sprintf(
				"ampcode.restrict-management-to-localhost: %t -> %t", oldCfg.AmpCode.RestrictManagementToLocalhost,
	...
	if oldMappings.hash != newMappings.hash {
		changes = append(
			changes,
			fmt.Sprintf("ampcode.model-mappings: updated (%d -> %d entries)", oldMappings.count, newMappings.count),
		)
	}
	if oldCfg.AmpCode.ForceModelMappings != newCfg.AmpCode.ForceModelMappings {
		changes = append(
			changes, fmt.Sprintf(
				"ampcode.force-model-mappings: %t -> %t", oldCfg.AmpCode.ForceModelMappings,
	...
	if !equalUpstreamAPIKeys(oldCfg.AmpCode.UpstreamAPIKeys, newCfg.AmpCode.UpstreamAPIKeys) {
		changes = append(
			changes, fmt.Sprintf(
				"ampcode.upstream-api-keys: updated (%d -> %d entries)", oldUpstreamAPIKeysCount,
				newUpstreamAPIKeysCount,
			),
		)
	}

	if entries, _ := OAuthExcludedModelChanges(
		oldCfg.OAuthExcludedModels, newCfg.OAuthExcludedModels,
	); len(entries) > 0 {
		changes = append(changes, entries...)
	}
	if entries, _ := oAuthModelAliasChanges(oldCfg.OAuthModelAlias, newCfg.OAuthModelAlias); len(entries) > 0 {
		changes = append(changes, entries...)
	}
	...
	if oldCfg.RemoteManagement.AllowRemote != newCfg.RemoteManagement.AllowRemote {
		changes = append(
			changes, fmt.Sprintf(
				"remote-management.allow-remote: %t -> %t", oldCfg.RemoteManagement.AllowRemote,
	...
	if oldCfg.RemoteManagement.DisableControlPanel != newCfg.RemoteManagement.DisableControlPanel {
		changes = append(
			changes, fmt.Sprintf(
				"remote-management.disable-control-panel: %t -> %t", oldCfg.RemoteManagement.DisableControlPanel,
	...
	if compat := openAICompatibility(oldCfg.OpenAICompatibility, newCfg.OpenAICompatibility); len(compat) > 0 {
	...
	if len(oldCfg.VertexCompatAPIKey) != len(newCfg.VertexCompatAPIKey) {
		changes = append(
			changes, fmt.Sprintf(
				"vertex-api-key count: %d -> %d", len(oldCfg.VertexCompatAPIKey), len(newCfg.VertexCompatAPIKey),
			),
		)
	} else {
		for i := range oldCfg.VertexCompatAPIKey {
			...
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(
					changes, fmt.Sprintf(
						"vertex[%d].base-url: %s -> %s", i, strings.TrimSpace(o.BaseURL), strings.TrimSpace(n.BaseURL),
					),
				)
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(
					changes, fmt.Sprintf(
						"vertex[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL),
					),
				)
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(
					changes, fmt.Sprintf(
						"vertex[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix),
					),
				)
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
			...
			if oldModels.hash != newModels.hash {
				changes = append(
					changes,
					fmt.Sprintf("vertex[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count),
				)
			}
			if !equalStringMap(o.Headers, n.Headers) {
...
func formatProxyURL(raw string) string {
	...
	if host == "" {
		...
		//noinspection HttpUrlsUsage — scheme prefix required for url.Parse to extract host:port.
